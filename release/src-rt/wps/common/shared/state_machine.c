 ...
 *
 * $Id: state_machine.c 241376 2011-02-18 03:19:15Z stakita $
 */
...
uint32
state_machine_send_ack(CStateMachine *sm, BufferObj *outmsg)
{
	uint32 err;

	err = reg_proto_create_ack(sm->mps_regData, outmsg, sm->mps_localInfo->version2);
	if (WPS_SUCCESS != err) {
...
uint32
state_machine_send_nack(CStateMachine *sm, uint16 configError, BufferObj *outmsg)
{
	uint32 err;

	err = reg_proto_create_nack(sm->mps_regData, outmsg, configError,
		sm->mps_localInfo->version2);
	if (WPS_SUCCESS != err) {
