 ...
 *
 * $Id: ap_eap_sm.c 241376 2011-02-18 03:19:15Z stakita $
 */
...
#define WPS_M2D_DISCOVERY()	\
	(apEapState->recv_msg_id == WPS_ID_MESSAGE_M1 || \
	(apEapState->sent_msg_id != 0 && apEapState->sent_msg_id != WPS_ID_MESSAGE_M2 && \
...
ap_eap_sm_sta_validate(char *eapol_msg)
{
	...
	if ((WpsNtohs((uint8*)&eapolHdr->length) <= EAP_HEADER_LEN) ||
		(eapolHdr->type != EAP_PACKET) ||
		(WpsNtohs((uint8*)&eapolHdr->length) <= EAP_HEADER_LEN) ||
...
ap_eap_sm_process_sta_msg(char *msg, int msg_len)
{
	...
	if (wpsEapHdr->type == EAP_TYPE_IDENTITY) {
		...
		else if (e_mode == EModeApProxyRegistrar) {
