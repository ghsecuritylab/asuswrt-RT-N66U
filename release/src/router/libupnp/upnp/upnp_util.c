 ...
 *
 * $Id: upnp_util.c 241389 2011-02-18 03:34:15Z stakita $
 */
#include <upnp.h>
#include <errno.h>
...
upnp_tlv_translate(UPNP_TLV *tlv)
{
	...
	case UPNP_TYPE_BOOL:
		...
		sprintf(buf, "%d", (char)tlv->val.ival);
		break;
	...
	case UPNP_TYPE_I2:
		tlv->val.ival &= 0xffff;
		sprintf(buf, "%d", (short)tlv->val.ival);
		break;

	case UPNP_TYPE_I4:
		sprintf(buf, "%ld", tlv->val.ival);
		break;

	case UPNP_TYPE_UI1:
		tlv->val.uval &= 0xff;
		sprintf(buf, "%u", (unsigned char)tlv->val.uval);
		break;

	case UPNP_TYPE_UI2:
		tlv->val.uval &= 0xffff;
		sprintf(buf, "%u", (unsigned short)tlv->val.uval);
		break;

	case UPNP_TYPE_UI4:
		sprintf(buf, "%lu", tlv->val.uval);
		break;
...
int
upnp_tlv_set_bin(UPNP_TLV *tlv, int data, int len)
{
	switch (tlv->type) {
	case UPNP_TYPE_BIN_BASE64:
		if (!data)
...
/* Set tlv */
int
upnp_tlv_set(UPNP_TLV *tlv, int data)
{
