...
void init_switch()
{
	...
	if(nvram_get_int("qos_enable") || nvram_get_int("url_enable_x") || nvram_get_int("ctf_disable_force")
        ...
        if( (fp = fopen("/tmp/switch_check", "r")) != NULL ) {
                while(fgets(buf, sizeof(buf), fp)) {
                        if(strstr(buf, "et interface not found")) {
                                _dprintf("No switch interface!!! Reboot...\n");
                                reboot(RB_AUTOBOOT);
                                return;
                        }
                }
