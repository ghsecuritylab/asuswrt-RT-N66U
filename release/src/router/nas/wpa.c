 ...
 *
 * $Id: wpa.c 248187 2011-03-23 06:53:23Z simonk $
 */
...
nas_wpa_calc_pmkid(wpa_t *wpa, nas_sta_t *sta)
{
	...
	uint8 data[128], digest[SHA1HashSize];
...
wpa_new_gtk_callback(bcm_timer_id td, wpa_t *wpa)
{
	...
	do {
		 ...
		 */
			...
			ts = wpa_set_itimer(nas->timer, &sta->td,
			                    (bcm_timer_cb)wpa_retransmission,
			                    (int)sta, sta->wpa_msg_timeout_s,
			                    sta->wpa_msg_timeout_ms);
			if (ts != ITIMER_OK)
...
wpa_insert_igtk(wpa_t *wpa, eapol_header_t *eapol, uint16 *data_len)
{
	...
	data_encap->length = (EAPOL_WPA2_ENCAP_DATA_HDR_LEN - TLV_HDR_LEN) +
	        EAPOL_WPA2_KEY_IGTK_ENCAP_HDR_LEN + wpa->igtk.len;
	...
	*(uint32 *)igtk_encap->ipn = wpa->igtk.ipn_lo;
	*(uint16 *)(igtk_encap->ipn + 4) = wpa->igtk.ipn_hi;

	bcopy(wpa->igtk.key, igtk_encap->key, wpa->igtk.len);
	len += wpa->igtk.len + EAPOL_WPA2_KEY_IGTK_ENCAP_HDR_LEN;

	/* return the adjusted data len */
	*data_len = len;
	printf("leave igtk_len=%d\n",  wpa->igtk.len);
	return TRUE;
}
#endif /* #ifdef MFP */
...
wpa_mic_error(wpa_t *wpa, nas_sta_t *msta, bool from_driver)
{
	time_t now, since;
	nas_t *nas = wpa->nas;
	nas_sta_t *sta; itimer_status_t ts;
#ifdef BCMDBG
	char eabuf[ETHER_ADDR_STR_LEN];
#endif
	...
	now = time(NULL);
	nas->MIC_failures++;
	dbg(nas, "MIC error %d at time %d, STA %s, %s", nas->MIC_failures,
	    (int) now, ether_etoa((uchar *)&msta->ea, eabuf),
	    (from_driver) ? "detected by driver" : "reported by STA");

	if (nas->flags & NAS_FLAG_SUPPLICANT) {
		/* Until this gets more thought do no harm. */
		dbg(nas, "Supplicant MIC error notices not yet handled");
		return;
	}
...
{
	eapol_wpa_key_header_t *body = (eapol_wpa_key_header_t *)eapol->body;
	unsigned char digest[2*MIC_KEY_LEN], mic[MIC_KEY_LEN];
	int mic_length;
	ushort key_info = ntohs(body->key_info);
	...
	case WPA_KEY_DESC_V1:
		hmac_md5(&eapol->version, mic_length, sta->suppl.eapol_mic_key,
		         MIC_KEY_LEN, digest);
		break;
	case WPA_KEY_DESC_V2:
		dbg(sta->nas, "using sha1 for V2");
		hmac_sha1(&eapol->version, mic_length,
		          sta->suppl.eapol_mic_key, MIC_KEY_LEN, digest);
		break;
#ifdef MFP
	case WPA_KEY_DESC_V3:
		dbg(sta->nas, "using aes cmac for V3");
		aes_cmac_calc(&eapol->version, mic_length,
		          sta->suppl.eapol_mic_key, MIC_KEY_LEN, digest);
		break;
#endif
	default:
		dbg(sta->nas, "unknown descriptor type %d", key_info);
		return FALSE;
	}

	/* compare against the MIC contents */
	if (bcmp(digest, mic, MIC_KEY_LEN) == 0) {
		dbg(sta->nas, "match");
		return (TRUE);
	} else {
		dbg(sta->nas, "fail");
...
{
	struct iovec frags;
	unsigned char mic[SHA1HashSize];	/* size of largest digest */
	...
	if (mic_length) {
		/* calculate the MIC  and copy it in */
		switch (sta->suppl.desc) {
		case WPA_KEY_DESC_V1:
			hmac_md5(&buffer[sizeof(struct ether_header)], mic_length,
			         sta->suppl.eapol_mic_key, MIC_KEY_LEN, mic);
			break;
		case WPA_KEY_DESC_V2:
			hmac_sha1(&buffer[sizeof(struct ether_header)], mic_length,
			          sta->suppl.eapol_mic_key, MIC_KEY_LEN, mic);
			break;
#ifdef MFP
		case WPA_KEY_DESC_V3:
			dbg(nas, "using aes cmac for V3");
			aes_cmac_calc(&buffer[sizeof(struct ether_header)], mic_length,
			          sta->suppl.eapol_mic_key, MIC_KEY_LEN, mic);
			break;
#endif
		default:
		...
		bcopy(mic, (char*)body->mic, MIC_KEY_LEN);
	}
	...
	uint32 *pmkc, nas_sta_t *sta)
{
	...
	if (len >= WPA_IE_SUITE_COUNT_LEN) {
		...
		if (!bcmp(mgmt->list[0].oui, oui, DOT11_OUI_LEN)) {
			...
			if (keyauth_type == KEYAUTH_SHA256) {
...
wpa_gen_igtk(wpa_t *wpa)
{
	...
	*(uint16 *)&data[data_len+sizeof(uint32)] = wpa->igtk.ipn_hi;
	...
	printf("WPA plugin IGTK, ID %d len  %d ", wpa->igtk.id, wpa->igtk.len);
	nas_set_key(wpa->nas, &wpa->nas->ea, wpa->igtk.key, wpa->igtk.len, wpa->igtk.id, 0,
