 ...
 *
 * $Id: wps_ie.c 241376 2011-02-18 03:19:15Z stakita $
 */
...
wps_set_prb_rsp_IE(int ess_id, char *ifname, CTlvSsrIE *ssrmsg)
{
	...
	/* Do not set PBC in Config Methods in unconfiged (OOB) */
	...
	/*
	 * Add in PF#3, in Probe Response(that should be an AP), Configuration
	 * Methods corresponds to the methods the AP supports "as an Enrollee" for
	 * "adding external Registrars".
	 * NOTE: the AP MUST not support PBC for "adding external Registrars".
	 * We will have trouble with wpa_supplicant "wps_reg" command
	 */
	if (b_wps_version2)
		data16 &= ~(WPS_CONFMET_VIRT_PBC | WPS_CONFMET_PHY_PBC);
	else
...
typedef struct _wps_vndr_ext_obj_t {
	...
	uint8	acPrefix[IFNAMSIZ + 1];
	uint8	acOSName[IFNAMSIZ + 1];
	int siType; /* WPS_IE_TYPE_SET_BEACON_IE or WPS_IE_TYPE_SET_PROBE_RESPONSE_IE */
	int siSize; /* Buffer Size */
	int siLen; /* Data Length */
	uint8 *pcData; /* Data Pointer to end of this struct if siSize > 0 during alloc */
}	wps_vndr_ext_obj_t;
	...
	for (ptPrev = NULL, ptObj = ptVEList; ptObj; ptObj = ptObj->ptNext) {
		if (ptObj == (wps_vndr_ext_obj_t *)ptToFree) {
			...
			if (ptObj->pcData && (ptObj->pcData !=
				((uint8 *)&ptObj->pcData + sizeof(ptObj->pcData))))
				free(ptObj->pcData);
			free(ptObj);
			return 1;
		}
...
wps_vndr_ext_obj_alloc(int siBufferSize, char *pcOSName)
{
	...
	if (siBufferSize == 0)
		...
		ptObj->pcData = (uint8 *)&ptObj->pcData + sizeof(ptObj->pcData);
...
wps_vndr_ext_obj_set_mode(void *ptVEObj, int siType, int boolActivate)
{
	...
	if (siType == WPS_IE_TYPE_SET_PROBE_RESPONSE_IE)
		wps_set_prb_rsp_IE(ptObj->acPrefix, ptObj->acOSName, FALSE);
	else if (siType == WPS_IE_TYPE_SET_BEACON_IE)
		wps_set_beacon_IE(ptObj->acPrefix, ptObj->acOSName, FALSE);
