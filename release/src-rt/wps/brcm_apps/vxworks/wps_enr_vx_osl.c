/*
 * WPS ENROLL thread (eCos platform dependent portion)
 *
 ...
 *
 * $Id: wps_enr_vx_osl.c 241376 2011-02-18 03:19:15Z stakita $
 */
...
wps_osl_get_mac(uint8 *mac)
{
	...
	if ((ret = ioctl(s, EIOCGADDR, (int)&ifr)) < 0) {
...
wps_osl_init(char *bssid)
{
	...
	if ((ret = ioctl(eap_fd, EIOCGADDR, (int)&ifr)) < 0) {
...
int
wpsenr_osl_set_wsec(void *credential)
{
	return wps_set_wsec(-1, if_name, credential, EModeClient);
}
