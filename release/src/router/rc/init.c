...
restore_defaults(void)
{
	...
	for(unit = WAN_UNIT_WANPORT1; unit < WAN_UNIT_MAX; ++unit){
		...
		for (t = router_state_defaults; t->name; t++) {
			...
			else if(unit == WAN_UNIT_WANPORT1) // let non-wan nvram to be set one time.
				nvram_set(t->name, t->value);
		}
	}
}
...
int init_nvram(void)
{
	...
	case MODEL_RTN55U:	
		...
		if(nvram_get_int("sw_mode")==SW_MODE_AP) {
			...
			nvram_set("lan_ifnames", "vlan0 vlan1 rai0 ra0");
			nvram_set("wan_ifnames", "");	
		}
			...
			// dual wan 
			// switch need to well-set for this config
			nvram_set("lan_ifnames", "vlan0 rai0 ra0");
			nvram_set("wan_ifnames", "vlan1 vlan2");
		}
#endif
		else { // router/default
			nvram_set("lan_ifnames", "vlan0 rai0 ra0");
...
//		nvram_set("boardflags", "0x310");
		...
		if(get_wans_dualwan()&WANSCAP_LAN) {
			...
			// switch need to well-set for this config
			nvram_set("lan_ifnames", "vlan1 eth1 eth2");
			nvram_set("wan_ifnames", "eth0 vlan3");
		}
		else if(get_wans_dualwan()&WANSCAP_2G) {
			nvram_set("lan_ifnames", "vlan1 eth2");
			nvram_set("wan_ifnames", "eth0 eth1");
		}
		else if(get_wans_dualwan()&WANSCAP_5G) {
			nvram_set("lan_ifnames", "vlan1 eth1");
			nvram_set("wan_ifnames", "eth0 eth2");
		}
		else {
			nvram_set("lan_ifnames", "vlan1 eth1 eth2");
			nvram_set("wan_ifnames", "eth0");
		...
		nvram_set_int("btn_radio_gpio", 13|GPIO_ACTIVE_LOW);
...
int init_main(int argc, char *argv[])
{
	int state, i;
	sigset_t sigset;
        int wl_check, boot_check; //Power on/off test
        char check_buf[4];
	...
	for (;;) {
		...
		case SIGUSR2:		/* START */
			...
			if(nvram_match("ate_power_on_off_enable", "1")) {
                                wl_check = nvram_get_int("ate_wl_check");
                                if (!wl_dev_exist()) {
                                        while(1) {
                                        dbG("wl dev fail at %d boot\n", wl_check);
                                        setAllLedOn();
                                        sleep(1);
                                        setAllLedOff();
                                        sleep(1);
                                        ...
                                        wl_check++;
                                        sprintf(check_buf, "%d", wl_check);
                                        nvram_set("ate_wl_check", check_buf);
                                        nvram_commit();
                                }
                        }
                        else {
			...
			if (restore_defaults_g)
			{
				...
				if ((nvram_get("wl0_channel") && atoi(nvram_get("wl0_channel"))) ||
					(nvram_get("wl1_channel") && atoi(nvram_get("wl1_channel"))) || get_model() == MODEL_RTN53)
					restart_wireless();
			}
#endif
#ifdef RTCONFIG_USB_PRINTER
			...
			if (wds_enable()) {
				...
				start_nas();
                        ...
                        if(nvram_match("ate_power_on_off_enable", "1")) {
                                ...
                                boot_check++;
                                sprintf(check_buf,"%d", boot_check);
                                nvram_set("ate_boot_check", check_buf);
                                if(boot_check < nvram_get_int("ate_reboot_count")) {
                                        dbG("System boot up success %d times, reboot...\n", boot_check);
	                                if(wl_check!=boot_check) { 
        	                                setAllLedOff();
                	                        while(1) {
                        	                dbG("wl check %d, boot check %d\n", wl_check, boot_check);
                                	        /* Switch Led On */
                                        ...
                                        nvram_set("ate_power_on_off_enable", "2");
                                        nvram_set("ate_reboot_count", "0");
                                        nvram_commit();
                                }
                        }
