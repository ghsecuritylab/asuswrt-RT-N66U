...
int add_multi_routes(){
	...
	for(unit = WAN_UNIT_WANPORT1; unit < WAN_UNIT_MAX; ++unit){ // Multipath
		...
		if(nvram_match("modem_mode", "2")){
			...
			// set the default gateway rule.
			if(strlen(cmd2) <= 0)
				strcpy(cmd2, "ip route replace default");

			wan_weight = nvram_get_int(strcat_r(prefix, "weight", tmp));
			if(wan_weight > 0){
				ptr = cmd2+strlen(cmd2);
				sprintf(ptr, " nexthop via %s dev %s weight %d", wan_gate, wan_if, wan_weight);
			}
		}
		else{
			// set the default gateway rule.
			memset(cmd, 0, 2048);
			sprintf(cmd, "ip route replace default nexthop via %s dev %s", wan_gate, wan_if);
			system(cmd);
