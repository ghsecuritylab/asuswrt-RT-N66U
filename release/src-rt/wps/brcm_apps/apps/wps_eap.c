 ...
 *
 * $Id: wps_eap.c 241376 2011-02-18 03:19:15Z stakita $
 */
...
#if defined(BCMWPSAP) && defined(UPNP_WFA_DEVICE)
#include <wps_upnp.h>
#endif
...
#endif /* BCMWPSAP && UPNP_WFA_DEVICE */
...
#ifdef WPS_AP_M2D
static int
wps_check_builtin_reg(int ess_id)
{
	char *value;
	char wps_reg[] = "essxxx_wps_reg";

	snprintf(wps_reg, sizeof(wps_reg), "ess%d_wps_reg", ess_id);
	value = wps_get_conf(wps_reg);
...
wps_eap_process_msg(char *buf, int buflen)
{
...
#ifdef WPS_AP_M2D
		else if ((ess_id = wps_eap_find_ess_id(ifname)) != -1 &&
			wps_check_builtin_reg(ess_id)) {
			/* clear station pin for AP/Router response M2D */
			wps_ui_set_env("wps_sta_pin", "");
			mode = WPSM_BUILTINREG_PROXY;
		}
#endif /* WPS_AP_M2D */
		else {
	 ...
	 */
		...
		TUTRACE((TUTRACE_ERR, "eapid = WPS_EAP_ID_REGISTRAR mode = WPSM_UNCONFAP\n"));
	}
	else {
		return ret;
	}
