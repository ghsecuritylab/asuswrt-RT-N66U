 ...
 *
 * $Id: wps_ecos_main.c 241376 2011-02-18 03:19:15Z stakita $
 */
#include <bn.h>
#include <wps_dh.h>
#include <md5.h>
...
wps_osl_validate_default()
{
	...
	if (!value) {
		...
		if (b_wps_version2) {
			/* make sure we set at least one display and one PBC */
			config_method |= WPS_CONFMET_VIRT_PBC  | WPS_CONFMET_VIRT_DISPLAY;
			sprintf(tmp, "0x%x", config_method);
			nvram_set("wps_config_method", tmp);
			dirty++;
		}
...
wps_osl_build_conf(void)
{
	...
	for (i = 0; i < 256; i ++) {
		...
		if (!ifname)
			continue;
#endif /* BCMWPSAP && __CONFIG_LIBUPNP__ */
...
#if defined(BCMWPSAP) && !defined(__CONFIG_LIBUPNP__)
	/* sleep one second for bcmupnp thread ready */
	WpsSleep(1);
#endif
