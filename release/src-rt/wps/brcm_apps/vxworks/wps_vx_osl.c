 ...
 *
 * $Id: wps_vx_osl.c 241376 2011-02-18 03:19:15Z stakita $
 */
...
wps_osl_arp_get(char *client_ip, unsigned char mac[6])
{
	...
	inet_aton(client_ip, &host_addr);
	arpCmd(SIOCGARP, &host_addr, mac, NULL);
...
wps_osl_upnp_send(int if_instance, UPNP_WPS_CMD *cmd, char *data)
{
	...
	WPS_HexDumpAscii("==> To UPnP", cmd->type, data, cmd->length);
