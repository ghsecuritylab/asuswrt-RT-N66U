 ...
 *
 * $Id: wps_wfi.c 241376 2011-02-18 03:19:15Z stakita $
 */
...
typedef struct wps_wfi_sta_nvinfo_s {
	union {
		struct { char acMAC[17], acFName[32], acNULL[1]; };
...
static char *
wps_wfi_get_ui_var(char *pcName)
{
	return wps_ui_get_env(pcName);
}
	...
	wps_wfi_bss_t *ptBSS) /* The BSS info */
{
	...
	memset(ptWFIVE, 0, sizeof(brcm_wfi_ie_t));
	...
	const char *pcIFPrefix)
{
	...
	strcpy(acName, pcIFPrefix);
	acName[strlen(pcIFPrefix) - 1] = 0; /* Get rid of _ at the end */
...
*/
{
	...
	stNVInfo.acMAC[sizeof(stNVInfo.acMAC)] = 0;
	...
	strncpy(stNVInfo.acFName, &pcSTAToInvite[sizeof(stNVInfo.acMAC)],
