 ...
 *
 * $Id: ap_api.c 241376 2011-02-18 03:19:15Z stakita $
 */
...
wps_init(void *bcmwps, devcfg *ap_devcfg, DevInfo *ap_devinfo)
{
	...
	if (gp_mc) {
		...
		strncpy(devcfg->m_nwKey, ap_devcfg->m_nwKey, ap_devcfg->m_nwKeyLen);
...
uint32
wps_genPin(char *devPwd, int devPwd_len)
{
	unsigned long PIN;
	...
	*/
	...
	if (WPS_SUCCESS != reg_proto_generate_psk(8, bufObj)) {
		TUTRACE((TUTRACE_ERR,
			"MC_GenerateDevPwd: Could not generate AP new random PIN\n"));
		buffobj_del(bufObj);
		return WPS_ERR_SYSTEM;
	}

	/* Compute the checksum */
	sprintf(local_devPwd, "%08u", *(uint32 *)bufObj->pBase);
	local_devPwd[7] = '\0';
...
wps_regUpnpForwardingCheck(RegSM *r, void *inbuffer, uint32 in_len,
	void *outbuffer, uint32 *out_len)
{
	uint32 msgType, err = WPS_SUCCESS, retVal = WPS_CONT;
