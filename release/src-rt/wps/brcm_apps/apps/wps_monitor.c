 ...
 *
 * $Id: wps_monitor.c 241376 2011-02-18 03:19:15Z stakita $
 */
#include <stdio.h>
#include <unistd.h>
#include <errno.h>
...
wps_dispatch()
{
	...
	if (wps_app->wksp) {
		if (ret != WPS_CONT) {
			TUTRACE((TUTRACE_INFO, "ret value = %d, wps_app ->mode = %d \n",
				ret, wps_app->mode));
