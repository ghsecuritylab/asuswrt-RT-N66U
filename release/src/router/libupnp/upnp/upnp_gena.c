 ...
 *
 * $Id: upnp_gena.c 241389 2011-02-18 03:34:15Z stakita $
 */
#include <upnp.h>
...
{
	UPNP_SCBRCHAIN	*scbrchain;
	UPNP_SUBSCRIBER	*subscriber, *next;
	UPNP_DEVICE *device = context->focus_ifp->device;
	struct in_addr addr;
	int err;
		...
		if (!ipaddr || subscriber->ipaddr.s_addr == addr.s_addr) {
			...
			if (sid) {
				...
				if (strcmp(sid, subscriber->sid) == 0) {
					err = submit_prop_event_message(
						context, service, subscriber);
					subscriber->seq++;
					if (err == -1 && device->notify) {
						/* WSC 2.0 */
						(*device->notify)(context, service, subscriber,
							DEVICE_NOTIFY_CONNECT_FAILED);
					}
					break;
				}
			}
...
get_unique_id(char *unique_id, unsigned int size)
{
	 ...
	 */
	sprintf(unique_id, "%08x-%04x-%04x-%04x-%04x%08x",
...
event_vars_shutdown(UPNP_CONTEXT *context, UPNP_SERVICE *service)
{
		 ...
		 statevar = statevar->next)
	{
		...
		for (prev = 0, event = statevar->event;
		     event;
		     prev = event, event = event->next) {
			if (event->ifp == context->focus_ifp) {
				if (prev == 0)
