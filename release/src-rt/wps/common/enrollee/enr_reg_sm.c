 ...
 *
 * $Id: enr_reg_sm.c 248684 2011-03-24 10:14:22Z kenlo $
 */
...
#define STR_ATTR_LEN(str, max)	((b_zpadding && strlen(str) < max) ? strlen(str) + 1 : strlen(str))
#else /* !WFA_WPS_20_TESTBED */
#define STR_ATTR_LEN(str, max)	(strlen(str))
			...
			if (WPS_SUCCESS != err) {
				if (err == RPROT_ERR_AUTH_ENC_FLAG) {
					...
					TUTRACE((TUTRACE_ERR, "ENRSM: receive rogue suspected MAC "
						"error: SendNack! %x\n", err));
					retVal = state_machine_send_nack(e->m_sm,
						WPS_ERROR_ROGUE_SUSPECTED, outmsg);
					goto out;
				}
...
reg_proto_process_m2(RegData *regInfo, BufferObj *msg, void **encrSettings)
{
	 ...
	 * We always save the coming M2 R-NONCE in reg2Nonce.
...
reg_proto_process_m8(RegData *regInfo, BufferObj *msg, void **encrSettings)
{
	...
	/* extract encrypted settings */
	{
		...
		if (regInfo->p_enrolleeInfo->version2 >= WPS_VERSION2) {
			...
			if (reg_proto_EncrSettingsCheck(regInfo->p_enrolleeInfo, *encrSettings)
				!= WPS_SUCCESS) {
				err = RPROT_ERR_INCOMPATIBLE;
				goto error;
			}
...
uint32
reg_proto_create_ack(RegData *regInfo, BufferObj *msg, uint8 wps_version2)
{
	uint8 data8;
	...
	uint8 wps_version2)
{
	...
	tlv_serialize(WPS_ID_ENROLLEE_NONCE, msg, regInfo->enrolleeNonce, SIZE_128_BITS);
	tlv_serialize(WPS_ID_REGISTRAR_NONCE, msg, regInfo->reg2Nonce, SIZE_128_BITS);
